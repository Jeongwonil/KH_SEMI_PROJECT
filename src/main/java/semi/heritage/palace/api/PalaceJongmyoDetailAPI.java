...
public class PalaceJongmyoDetailAPI {
	public static String palace_Jongmyo_Sangse_XML_URL = "";
	// ?serial_number=0&detail_code=0&gung_number=3

	public static void main(String[] args) {
		PalaceJongmyoDetailAPI.callPalaceJongmyoSangseListByXML();

	}

	public static List<PalaceJongmyoDetail> callPalaceJongmyoSangseListByXML() {
		List<PalaceJongmyoDetail> list = new ArrayList<>();

		for (int seq = 0; seq < 127; seq++) {
			palace_Jongmyo_Sangse_XML_URL = PalaceJongmyoAPI.callPalaceJongmyoListByXML().get(seq).getLink();
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						for (int j = 0; j < nList2.getLength(); j++) {
							...
							if (node2.getNodeType() == Node.ELEMENT_NODE) {
								...
								PalaceJongmyoDetail pjs = new PalaceJongmyoDetail(seq_pj_ss, serial_number,
