...
public class PalaceJongmyoDetailMovieAPI {
	public static String palace_Jongmyo_Sangse_Movie_XML_URL = "";

	public static void main(String[] args) {
		PalaceJongmyoDetailMovieAPI.callPalaceJongmyoSangseListByXML();

	}

	public static List<PalaceJongmyoDetailMovie> callPalaceJongmyoSangseListByXML() {

		List<PalaceJongmyoDetailMovie> list = new ArrayList<>();

		for (int seq = 0; seq < 127; seq++) {
			palace_Jongmyo_Sangse_Movie_XML_URL = PalaceJongmyoAPI.callPalaceJongmyoListByXML().get(seq)
					.getLink();
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						for (int j = 0; j < nList2.getLength(); j++) {
							...
							if (node2.getNodeType() == Node.ELEMENT_NODE) {
								...
								PalaceJongmyoDetailMovie pjsm = new PalaceJongmyoDetailMovie(serial_number,
